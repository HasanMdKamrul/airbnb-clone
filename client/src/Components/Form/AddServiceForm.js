...
import { addHome } from "../../Apis/homeApis";
import { imageUpload } from "../../Apis/imageUpload";
import { AuthContext } from "../../contexts/AuthProvider";

const AddServiceForm = () => {
  const { user } = useContext(AuthContext);
  const [arivalDate, setArrivalDate] = useState(new Date());
  const [deparatureDate, setDeparatureDate] = useState(
    new Date(arivalDate + 24 * 60 * 60 * 1000)
  );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const homeData = {
        ...
        from: format(arivalDate, "P"),
        to: format(deparatureDate, "P"),
      };
              ...
              <div className="shadow-md rounded-md my-2 p-3 flex justify-between items-center">
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) => setDeparatureDate(date)}
                    className="w-2/3"
                    dateFormat="P"
                  />
                </div>
