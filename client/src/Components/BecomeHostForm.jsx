...
import { updateRoleData } from "../Apis/userCreationAndToken";
...
const BecomeHostForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const imageDoc = form.image.files[0];

    // ** Image upload to imageBB

    try {
      const imageUrl = await imageUpload(imageDoc);
      const hostData = {
        location,
        hostDocImage: imageUrl,
        role: "requested",
        email: user?.email,
      };

      //   ** Become host korte chaile ekhon amra user to oi j host hoite chaise take khuje ber kore tar role update kore dibo

      //   ** update role in user data

      await updateRoleData(hostData);
    } catch (error) {
